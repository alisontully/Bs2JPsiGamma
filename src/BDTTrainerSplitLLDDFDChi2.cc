...
#include "../interface/BDTTrainerSplitLLDDFDChi2.h"

using namespace std;
using namespace TMVA;

BDTTrainerSplitLLDDFDChi2::BDTTrainerSplitLLDDFDChi2(TString _name):
	BaseAnalyser(_name),
	outfilename("MVATrainingOutputFDChi2.root"),
	evCount(0),
	numberOfBDTs(5)
{}

BDTTrainerSplitLLDDFDChi2::~BDTTrainerSplitLLDDFDChi2(){}

void BDTTrainerSplitLLDDFDChi2::Init(Looper *l){
	cout << Form("%-30s","BDTTrainerSplitLLDDFDChi2::Init()") << " " << "Initialising Analyser (" << name << ")." << endl;
  ...
  factoryContainer["DD"] = vector<TMVA::Factory*>();
	for (int b=0; b<numberOfBDTs; b++){
		factoryContainer["LL"].push_back(new Factory(Form("%sFactoryRegieVarsLL%d",name.Data(),b),outFile,factoryOptions));
		factoryContainer["DD"].push_back(new Factory(Form("%sFactoryRegieVarsDD%d",name.Data(),b),outFile,factoryOptions));
	}
...
void BDTTrainerSplitLLDDFDChi2::Term(Looper *l){
  ...
  cout << Form("%-30s","BDTTrainerSplitLLDDFDChi2::Term()") << " " << "Training output written to: " << outFile->GetName() << endl;
	//cout << Form("%-30s","BDTTrainerSplitLLDDFDChi2::Term()") << " " << "Training output written to: " << outFiles["LL"]->GetName() << endl;
	//cout << Form("%-30s","BDTTrainerSplitLLDDFDChi2::Term()") << " " << "Training output written to: " << outFiles["DD"]->GetName() << endl;

	cout << Form("%-30s","BDTTrainerSplitLLDDFDChi2::Term()") << " " << "Terminating Analyser (" << name << ")." << endl;
}

bool BDTTrainerSplitLLDDFDChi2::AnalyseEvent(Looper *l){
